...
export class Graph {
     ...
     */
    addVertex(props: VertexProperties) {
    ...
    /*
     * Each vertex has an id and so edges can be defined as sets of two
     * vertices like { A, B } for example. This method takes the id of the
     * vertex and returns the corresponding Vertex object from the supplied list.
     */
    lookupVertex(graphVertices, id) {
        "use strict";
    ...
    /*
     * Returns the id of the lowest degree vertex of a given graph
     *
     */
    findLowestDegreeVertex(vertexObjectList, edgeObjectList) {
        ...
        vertexObjectList.forEach(vertex => {
            let d = this.deg(vertex.id, edgeObjectList);
            if (d < deg) {
                deg = d;
                lowest = vertex;
            }
